...
public class Clipper {
  ...
  private static void cutRightEdge(Vertex3D p1, Vertex3D p2, ArrayList<Vertex3D> list){
    ...
    if(p1.getX() <= xMax && p2.getX() <= xMax){ //in  in
      ...
      double camXSlope = (p2.getCameraPoint().getX() - p1.getCameraPoint().getX())/(p2.getX() - p1.getX());
      double camYSlope = (p2.getCameraPoint().getY() - p1.getCameraPoint().getY())/(p2.getX() - p1.getX());

      y = p1.getY() + xToEdge*slope; //add the difference
      z = p1.getZ() + xToEdge*zSlope;

      camX = p1.getCameraPoint().getX() + xToEdge*camXSlope;
      camY = p1.getCameraPoint().getY() + xToEdge*camYSlope;

      Vertex3D newVertex = new Vertex3D(xMax, y, z, p1.getColor());
      newVertex.setCameraPoint(new Point3DH(camX, camY, z));
      if(p2.hasNormal()) newVertex.setNormal(p2.getNormal());
      list.add(newVertex);
    }
      ...
      double camXSlope = (p1.getCameraPoint().getX() - p2.getCameraPoint().getX())/(p1.getX() - p2.getX());
      double camYSlope = (p1.getCameraPoint().getY() - p2.getCameraPoint().getY())/(p1.getX() - p2.getX());


      y = p2.getY() + xToEdge*slope; //add the difference
      z = p2.getZ() + xToEdge*zSlope;

      camX = p2.getCameraPoint().getX() + xToEdge*camXSlope;
      camY = p2.getCameraPoint().getY() + xToEdge*camYSlope;

      Vertex3D newVertex = new Vertex3D(xMax, y, z, p1.getColor());
      newVertex.setCameraPoint(new Point3DH(camX, camY, z));
      if(p1.hasNormal()) newVertex.setNormal(p1.getNormal());
      list.add(newVertex);
      list.add(p2);
    }
  }
  ...
  private static void cutTopEdge(Vertex3D p1, Vertex3D p2, ArrayList<Vertex3D> list){
    ...
    if(p1.getY() <= yMax && p2.getY() <= yMax){ //in  in
      ...
      double camXSlope = (p2.getCameraPoint().getX() - p1.getCameraPoint().getX())/(p2.getY() - p1.getY());
      double camYSlope = (p2.getCameraPoint().getY() - p1.getCameraPoint().getY())/(p2.getY() - p1.getY());

      x = p1.getX() + yToEdge*xSlope; //add the difference
      z = p1.getZ() + yToEdge*zSlope;

      camX = p1.getCameraPoint().getX() + yToEdge*camXSlope;
      camY = p1.getCameraPoint().getY() + yToEdge*camYSlope;
      ...
      double camXSlope = (p1.getCameraPoint().getX() - p2.getCameraPoint().getX())/(p1.getY() - p2.getY());
      double camYSlope = (p1.getCameraPoint().getY() - p2.getCameraPoint().getY())/(p1.getY() - p2.getY());

      x = p2.getX() + yToEdge*xSlope; //add the difference
      z = p2.getZ() + yToEdge*zSlope;

      camX = p2.getCameraPoint().getX() + yToEdge*camXSlope;
      camY = p2.getCameraPoint().getY() + yToEdge*camYSlope;
...
//-200 is the usual minimum
  private static void cutFarEdge(Vertex3D p1, Vertex3D p2, ArrayList<Vertex3D> list){
    ...
    if(p1.getZ() >= zMin && p2.getZ() >= zMin){ //in  in
      ...
      double camXSlope = (p1.getCameraPoint().getX() - p2.getCameraPoint().getX())/(p1.getZ() - p2.getZ());
      double camYSlope = (p1.getCameraPoint().getY() - p2.getCameraPoint().getY())/(p1.getZ() - p2.getZ());

      x = p2.getX() + zToEdge*xSlope; //both have same x value
      y = p2.getY() + zToEdge*ySlope;

      camX = p2.getCameraPoint().getX() + zToEdge*camXSlope;
      camY = p2.getCameraPoint().getY() + zToEdge*camYSlope;

      Vertex3D newVertex = new Vertex3D(x, y, zMin, p2.getColor());
      newVertex.setCameraPoint(new Point3DH(camX, camY, zMin));
      if(p2.hasNormal()) newVertex.setNormal(p2.getNormal());
      list.add(newVertex);
    }
      ...
      double camXSlope = (p2.getCameraPoint().getX() - p1.getCameraPoint().getX())/(p2.getZ() - p1.getZ());
      double camYSlope = (p2.getCameraPoint().getY() - p1.getCameraPoint().getY())/(p2.getZ() - p1.getZ());

      x = p1.getX() + zToEdge*xSlope; //both have same x value
      y = p1.getY() + zToEdge*ySlope;

      camX = p1.getCameraPoint().getX() + zToEdge*camXSlope;
      camY = p1.getCameraPoint().getY() + zToEdge*camYSlope;
  ...
  private static void cutNearEdge(Vertex3D p1, Vertex3D p2, ArrayList<Vertex3D> list){
    ...
    if(p1.getZ() <= zMax && p2.getZ() <= zMax){ //in  in
      ...
      newVertex.setCameraPoint(new Point3DH(camX, camY, zMax));
      if(p2.hasNormal()) newVertex.setNormal(p2.getNormal());
      list.add(newVertex);
    }
